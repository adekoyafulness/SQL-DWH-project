	...
	BEGIN TRY
		...
		SELECT
			...
			CASE WHEN UPPER (TRIM (cst_marital_status)) = 'S' THEN 'Single'
				 ...
				 ELSE 'n/a'
			END cst_marital_status,  -- Normalizes marital status values to readable format
			CASE WHEN UPPER (TRIM (cst_gndr)) = 'F' THEN 'Female'
				 WHEN UPPER (TRIM (cst_gndr)) = 'M' THEN 'Male'
				 ELSE 'n/a'
			END cst_gndr,  -- Normalizes gender values to readable format
			cst_create_date
		FROM (
				SELECT
				*,
		...
		SELECT
			CASE WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid))
				 ELSE cid
			END cid, -- Handled invalid values
